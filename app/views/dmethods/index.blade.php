			...
			<div class="row">
				<div class='col-md-12'>
						...
						<div class="dashboard-stat green">
							...
							<a href="{{ URL::to('dmethods/create') }}" class="more">
							Add Dmethod <i class="m-icon-swapright m-icon-white"></i>
							</a>
						</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="well">
						...
						<p>{{ link_to_route('dmethods.create', 'Add new dmethod') }}</p>
				...
				<th>Desc</th>
								...
								<tbody>
									@foreach ($dmethods as $dmethod)
										<tr>
											<td>{{{ $dmethod->name }}}</td>
					<td>{{{ $dmethod->dtype_id }}}</td>
					<td>{{{ $dmethod->label }}}</td>
					<td>{{{ $dmethod->desc }}}</td>
                    <td>{{ link_to_route('dmethods.edit', 'Edit', array($dmethod->id), array('class' => 'btn btn-info')) }}</td>
                    <td>
                        {{ Form::open(array('method' => 'DELETE', 'route' => array('dmethods.destroy', $dmethod->id))) }}
