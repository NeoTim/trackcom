			...
			<div class="row">
				<div class='col-md-12'>
						...
						<div class="dashboard-stat green">
							...
							<a href="{{ URL::to('trucks') }}" class="more">
							View Groups <i class="m-icon-swapright m-icon-white"></i>
							</a>
						</div>
				</div>
			</div>
			...
			<div class="row">
				<div class='col-md-12'>
					<div class='well'>
						...
						<p>{{ link_to_route('grps.index', 'Return to all trucks') }}</p>
				...
				<th>Orders</th>
							...
							<tbody>
								<tr>
									<td>{{{ $grp->truck }}}</td>
					<td>{{{ $grp->orders }}}</td>
                    <td>{{ link_to_route('trucks.edit', 'Edit', array($grp->id), array('class' => 'btn btn-info')) }}</td>
                    <td>
                        {{ Form::open(array('method' => 'DELETE', 'route' => array('grps.destroy', $grp->id))) }}
