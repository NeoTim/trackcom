			...
			<div class="row">
				<div class='col-md-12'>
						...
						<div class="dashboard-stat green">
							...
							<a href="{{ URL::to('grps/create') }}" class="more">
							Add Group <i class="m-icon-swapright m-icon-white"></i>
							</a>
						</div>
				</div>
			</div>
								...
								<tbody>
									@foreach ($grps as $grp)
										<tr>
											<td>{{{ $grp->truck }}}</td>
					<td>{{{ $grp->orders }}}</td>
                    <td>{{ link_to_route('grp.edit', 'Edit', array($grp->id), array('class' => 'btn btn-info')) }}</td>
                    <td>
                        {{ Form::open(array('method' => 'DELETE', 'route' => array('grp.destroy', $truck->id))) }}
